...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(sid.equals("null")) { // se non ho il sid sicuramente non ho neanche l'uid
            ...
            InternetCommunication internetCommunication = new InternetCommunication(this);
            /* richiedo gli sponsor */
            internetCommunication.getSponsors(
                    response -> {
                        MyModel.getSingleton().initSponsorsFromJSON((JSONObject)response);
                        Log.d("Debug", "esamefebbraio numero sponsors " + MyModel.getSingleton().getSponsorsSize());
                        List<Sponsor> sponsorList = MyModel.getSingleton().getSponsorList();
                        for(Sponsor s: sponsorList)
                            Log.d("Debug", "esamefebbraio nome: " + s.getNome() + ", lat: " + s.getLat() + ", lon: " + s.getLon());
                    },
                    error -> {
                        Log.d("Debug", "Error: " + error.toString());
                        InternetCommunication.showNetworkError(this, true);
