...
public class MyModel {
    ...
    public synchronized void initSponsorsFromJSON(JSONObject sponsors){
        try {
            this.sponsorList = new ArrayList<>();
            JSONArray sponsorsJSON = sponsors.getJSONArray("sponsors");
            for(int i=0; i<sponsorsJSON.length(); i++){
                JSONObject sponsor = (JSONObject) sponsorsJSON.get(i);
                Sponsor s = new Sponsor(
                        sponsor.getString("name"),
                        sponsor.getString("url"),
                        sponsor.getString("text"),
                        sponsor.getString("lat"),
                        sponsor.getString("lon"),
                        sponsor.getString("image")
                );
    ...
    public synchronized int getSponsorsSize(){
        return this.sponsorList.size();
    }

    public synchronized List<Sponsor> getSponsorList() {
        return sponsorList;
    }

    public synchronized Sponsor getSponsor(int index) {
        if(index >= 0 && index < sponsorList.size())
